...
function playSine(note, duration = 0.5) {
  ...
  // 페이드 아웃: 음 끝나기 100ms 전부터 볼륨을 0으로 선형 감소
  const fadeOutTime = 0.1;
  const fadeStartTime = audioContext.currentTime + duration - fadeOutTime;
  gainNode.gain.setValueAtTime(initialVolume, fadeStartTime);
